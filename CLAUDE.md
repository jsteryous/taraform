...
- [ ] **Phone contact sync is single-operator, not a product feature** (added 2026-08-04). `scripts/phone-sync.mjs` pushes contacts to Google Contacts nightly so callers show a name; see `scripts/PHONE_SYNC.md`. It reads ONE Google refresh token + ONE Supabase login from repo secrets, so it serves only the repo owner — other users (`alex@rebbadvisors.com`, `devankashi3@gmail.com`) get nothing from signing in. Deliberate v1, not an oversight. **Full multi-user design is now written up in `scripts/PHONE_SYNC_MULTIUSER.md`** (2026-08-04) — Supabase Edge Functions + pg_cron + Vault-held refresh tokens, ~2½ days, still $0. It corrects the earlier sketch here: a purely browser-side PKCE flow *cannot* do this, because GIS token-model renewal requires a user gesture and so can only produce a manual "Sync now" button. Revisit only when a second user actually asks.
