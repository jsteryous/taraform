...
- [ ] **Phone contact sync is single-operator, not a product feature** (added 2026-08-04). `scripts/phone-sync.mjs` pushes contacts to Google Contacts nightly so callers show a name; see `scripts/PHONE_SYNC.md`. It reads ONE Google refresh token + ONE Supabase login from repo secrets, so it serves only the repo owner — other users (`alex@rebbadvisors.com`, `devankashi3@gmail.com`) get nothing from signing in. Deliberate v1, not an oversight. Multi-user path, still $0 and still no always-on server: (1) a `google_contact_sync` table, one RLS-gated row per user holding their refresh token; (2) a "Connect Google Contacts" settings button doing browser-side PKCE OAuth (no client secret client-side); (3) the same nightly Action loops over those rows instead of reading env. Roughly a day's work; **settle how third-party refresh tokens are protected at rest before it holds anyone else's contacts.** Revisit only when a second user actually asks.
