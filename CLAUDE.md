...
**CSV import:** Duplicate detection uses Map-based lookups (O(n+m)) — do not revert to `.filter()` scan (O(n²) freezes on large imports). Bulk inserts chunked at 500 rows. `email2` is fallback if `email1` is empty — intentional, one email per contact.
